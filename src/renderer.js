function load() {
    ...
    const func = async () => {
        xs.cellText(0, 2, "node").reRender();
        xs.cellText(1, 2, window.versions.node()).reRender();

        xs.cellText(0, 3, "chrome").reRender();
        xs.cellText(1, 3, window.versions.chrome()).reRender();

        xs.cellText(0, 4, "electron").reRender();
        xs.cellText(1, 4, window.versions.electron()).reRender();

        xs.cellText(0, 5, "ping").reRender();
        const response = await window.versions.ping();
        xs.cellText(1, 5, response).reRender();
    };
    func();
}
